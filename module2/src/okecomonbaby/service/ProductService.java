package okecomonbaby.service;

import okecomonbaby.model.Product;
import okecomonbaby.repository.ProductRepository;

import java.util.List;
import java.util.Scanner;

public class ProductService implements IProductService {
    private ProductRepository productRepository = new ProductRepository();
    private Scanner scanner = new Scanner(System.in);

    @Override
    public void display() {
        List<Product> products = productRepository.getAll();
        for (Product p : products) {
            System.out.println(p);
        }

    }

    @Override
    public void add() {
        System.out.println("Nhập ì mà bạn muốn thêm");
        String id = scanner.nextLine();
        System.out.println("Nhập tên sẩn phẩm mà bạn muốn thêm");
        String name = scanner.nextLine();
        System.out.println("nhập số lượng sản phẩm");
        int quantity = Integer.parseInt(scanner.nextLine());
        System.out.println("nhập giá sản phẩm muốn thêm");
        float price = Float.parseFloat(scanner.nextLine());
        System.out.println("Nhâp mô tả sản phẩm");
        String description = scanner.nextLine();
        Product product = new Product(id, name, quantity, price, description);
        productRepository.add(product);
        System.out.println("thêm mới oke");
    }

    @Override
    public void delete() {
        System.out.println("nhập id sản phẩm mà bạn muốn xoá");
        String id = scanner.nextLine();
        Product product = productRepository.getById(id);
        if (product == null) {
            System.out.println("Không tìm thấy sản phẩm muốn xoá");
        } else {
            System.out.println("bạn có muốn xoá sản phẩm với tên" + product.getNameProduct());
            System.out.println("1. xoá");
            System.out.println("2.thêm");
            int choice = Integer.parseInt(scanner.nextLine());
            if (choice == 1) {
                productRepository.delete(product);
                System.out.println("xoá oke");
            }
        }
    }
}
